...
class WH_SSO{
	...
	function get_url() {
		...
		$salt = $salt ? $salt : substr(str_shuffle(str_repeat('abcdefghijklmnopqrstuvwxyz0123456789',5)),0,32);
	...
	function validate_token($token = false, $salt = false) {
		...
		$token = $token ? $token : $_GET[ $this->_config['token_key'] ];
		$salt = $salt ? $salt : $_GET[ $this->_config['salt_key'] ];
		if(!$token) return 0;
